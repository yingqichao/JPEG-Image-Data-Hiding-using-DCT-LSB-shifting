...
public class Calctool {
	...
	public int indexOf_bisection(int[] arr,int key){
		//如果不会有重复或者只想找到第一个，或者只想查询是否存在，用这个
		int low = 0;int high = arr.length - 1;int middle = 0;			//定义middle
	...
	public int[] huffmanDecoder(char[] f,int flag,int[][] tree,int row,int col){
		...
		for(i=int_min(tree[0]);i<=int_max(tree[0]);i++){
			index = indexOf_bisection(tree[1],bin2dec_str(f,flag,i));
	...
	public int bin2dec_string(String a,int flag,int len){
		int res = 0;int i;
		for (i=0;i<len;i++){
			if(a.charAt(flag+i)==49){
				res += Math.pow(2, len-1-i);
			}
		}
		return res;
	...
	public byte[] str2byte_control(String a,int[] rst){
		...
		char[] b = new char[len];byte[] res = new byte[len/8+10000];
		for(int i=0;i<len;i++){
		...
		for(int i=0;i<len/8;i++){
			buff=bin2dec_str(b,i*8,8);if(i!=len/8-1) {buff2=bin2dec_str(b,(i+1)*8,8);}
	...
	public String int2bit(int a,int len){
		//将码字对应的int重新变回长度为len的码字
		StringBuilder res = new StringBuilder();
		for(int i=0;i<len;i++){
			if(a>=Math.pow(2, len-1-i)){a -= Math.pow(2, len-1-i);res.append((char)49);}
			else{res.append((char)48);}
		}
		return res.toString();
	}
	
	public char[] int2char(int a,int len){
		//将码字对应的int重新变回长度为len的码字
		char[] res = new char[len];
		for(int i=0;i<len;i++){
			if(a>=Math.pow(2, len-1-i)){a -= Math.pow(2, len-1-i);res[i]=(char)49;}
			else{res[i]=(char)48;}
		}
		return res;
	}
